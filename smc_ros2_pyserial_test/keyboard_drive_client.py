...
and publishes them to control the smc_driver 
via the smc_drive_server_node.
...
class KeyboardDriveClient(Node):
    def __init__(self):
        super().__init__(node_name="keyboard_drive_client_node") # initialize the node name

        self.defaultAngVel = process_args_vel()
        self.angVelA = 0.000
        self.angVelB = 0.000

        self.send_cmd = self.create_publisher(Float32MultiArray, 'smc/cmd_vel', 10)
...
def main(args=None):
    ...
    # Create the node
    keyboard_drive_client = KeyboardDriveClient()

    # spin the node so the call back function is called
    rclpy.spin(keyboard_drive_client)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    keyboard_drive_client.destroy_node()
